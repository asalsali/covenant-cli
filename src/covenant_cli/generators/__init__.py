...
def generate_manager_file(service_spec: dict, pipeline_step: dict) -> str:
    ...
    for agent in service_spec["agents"]:
        agent_runs.append(
            f'            # Phase 2: Merge upstream data into tool_results\n'
            f'            upstream_tools = request.get("tool_results", {{}})\n'
            f'            if upstream_tools:\n'
            f'                for k, v in upstream_tools.items():\n'
            f'                    if k not in tool_results:\n'
            f'                        tool_results[k] = v\n'
            f'            upstream_sections = request.get("sections", [])\n'
            f'            # Build analysis prompt with real data\n'
            f'            analysis_input = f"Analyze the provided data.\\n\\nUser request: {{request.get(\'query\', \'\')}}\\n\\nData:\\n{{json.dumps(tool_results, indent=2, default=str)}}\\n\\n"\n'
            f'            if upstream_sections:\n'
            f'                analysis_input += f"Prior analysis:\\n{{json.dumps(upstream_sections, indent=2, default=str)}}\\n\\n"\n'
            f'            analysis_input += "Format with ## headers for sections, - bullets for lists, KEY: VALUE for metrics."\n'
            f'\n'
